...
/*
  ...
    xmlhttp.send("isonline=" + a);
  }
  
  function getsession(y) { 
    var a = y;
    var xmlhttp = new XMLHttpRequest();
    xmlhttp.onreadystatechange = function() {
      alert(this.readyState+" "+this.status);
    ...
    xmlhttp.open("POST", "getsession.php", true);
    xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xmlhttp.send("getsession=" + a);
  }

  function delsession(z) { 
    var a = z;
    var xmlhttp = new XMLHttpRequest();
    xmlhttp.onreadystatechange = function() {
      alert(this.readyState+" "+this.status);
    };
    xmlhttp.open("POST", "delsession.php", true);
    xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xmlhttp.send("delsession=" + a);
}
  var q = getsession("isonline");
  socket.emit('isonline', q);
*/
  ...
  socket.on('new message', (data, cid) => {
    ...
    if (data.send == uid && data.rece == who.id) {
      ...
      var $img = '<button class="img" style="background-image:url('+"'./pics/"+data.spic+"'"+');"></button>';
  ...
  socket.on('sign in', (data, cid) => {
    ...
    $('#chatroom').html("Let's chat!<span class='clear'></span>");
